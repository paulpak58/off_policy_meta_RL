...
Adapted code is inspired from garage docs https://github.com/rlworkgroup/garage/blob/master/docs/user/algo_pearl.md
...
class PEARL(MetaRLAlgorithm):
    ...
    Args:
        env (list[Environment]): batch of sampled environment updates(
            ...
            them with new tasks.
        policy_class (type): class implementing ContextConditionedPolicy
        encoder_class (garage.torch.embeddings.ContextEncoder): encoder class context-conditioned policy.
        ...
        num_test_tasks (int or None): num tasks for testing.
        latent_dim (int): size of latent context vector.
        encoder_hidden_sizes (list[int]): out dim of dense layer(s) of the context encoder.
        ...
        optimizer_class (type): optimizer for training networks.
        use_information_bottleneck (bool): False = deterministic latent context
        use_next_obs_in_context (bool): True = use next observation in distinguishing between tasks.
        ...
        num_steps_posterior (int): num transitions to obtain per task with z ~ posterior.
        num_extra_rl_steps_posterior (int): num additional transitions to obtain per task with z ~ posterior that are only used to train the policy and NOT the encoder.
        batch_size (int): num transitions in RL batch.
        embedding_batch_size (int): num transitions in context batch.
        embedding_mini_batch_size (int): num transitions in mini context batch; should be same as embedding_batch_size for non-recurrent encoder.
        discount (float): RL discount factor.
        replay_buffer_size (int): max samples in replay buffer.
        reward_scale (int): reward scale.
        update_post_train (int): resample context frequency when obtaining data during training (in episodes).
    """
    ...
    def __init__(
        ...
        num_test_tasks = test_env_sampler.n_tasks if num_test_tasks is None else num_test_tasks
        if num_test_tasks is None:
            raise ValueError('Tasks for test time are required')
    ...
    def train(self, trainer):
        ...
        for _ in trainer.step_epochs():
            ...
            logger.log('Training...')
            self._train_once()              # sample train tasks
            trainer.step_itr += 1
            logger.log('Evaluating...')
            self._policy.reset_belief()
            self._evaluator.evaluate(self)  # sample test tasks
